...
void print_info(result_t *result)
{
...
int main(int argc, char **argv )
{
	...
	if ( argc == 2) {
		...
		if ( !err ) {
			...
			if ( tidy_result >= 0 ) {
				...
				if ( tidy_result >= 0 ) {
					...
					if ( tidy_result >= 0 ) {
						results = walkNode(tdoc, results); /* walk the node tree */ 
					} else {
						printf("tidyRunDiagnostics result: %d\n", tidy_result);
					}
				} else {
					printf("tidyCleanAndrepair result: %d\n", tidy_result);
				}
			} else {
				printf("tidyParseBuffer result: %d\n", tidy_result);
			}
		}
		else {
			printf("errbuf output:\n%s\n", curl_errbuf);
		}
		...
		dprintf("results=0x%08x\n", (unsigned int) results);
		if (results) {
			g_slist_foreach(results, (GFunc)print_info, NULL);
